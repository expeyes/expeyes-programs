...
// The global variable below are an issue, if we want to make a DLL out of this. 
//For the time being compile this file with the main program.
...
// Search for the reply "ejx.x" (version x.x) on the specified port. Returns file handle or -1
{
  ...
  // Checking whether the device is already used by another process. Need better code.
  sprintf(ss,"fuser %s",device);
  int ret = system(ss);
  if(ret == 0) 
	{
  ...
  fd = open (device, O_RDWR | O_NOCTTY); // If no other process using, we try opening it
  if (fd < 0)
	{
  ...
  newtio.c_cc[VTIME] = 40; 			// Timeout for read in deciseconds
  ...
  sread(5,ss);
  if(!strncmp((char*)ss,"ej",2))	// found Some version of EYES Junior hardware
		return fd;
  return -1;
}
...
#else
	#define MAXPORT		4
	char *devlist[MAXPORT] = {"/dev/ttyACM0","/dev/ttyACM1","/dev/ttyACM2","/dev/ttyACM3"}; 
	for(k= 0; k < MAXPORT; ++k)
		{
...
byte get_voltage_time(byte ch, int* t, float* v)
	{
	u16 iv;
	byte res;
	time_t tval; 
...
byte set_pulsewidth(u16 width)
	...
	// Also used by usound() and the elable_pulse_high/low() functions
	byte res[1];
