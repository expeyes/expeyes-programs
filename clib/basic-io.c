...
// Read voltage from a channel, with time stamp
if(get_voltage_time(3, &tm, &v)) exit(1);
printf("Time = %d Voltage at IN1 is %5.3f V\n", tm, v);

// Reads Logic level at a channel. Connect PVS to IN1
if(get_state(3, &st)) exit(1);
printf("Level at IN1 is %d\n", st);

// Measures the capacitance on IN1
if(measure_cap(&pf)) exit(1);
printf("Capacitance of IN1 is %f\n", pf);
