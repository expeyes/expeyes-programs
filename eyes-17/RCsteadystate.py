...
def update():
	...
	try:
		...
		if fa != None and fb != None and fc != None:
			...
			Fit3.config(text = _('A1-A2:Voltage across C = %5.2f V') %(a2))
...
def calc():
	try:
		...
		if C != 0:
			...
			Xc = 0.0
		dphi = math.atan( Xc/R)*180./math.pi
		s = _('XC = %5.1f  \nDphi = %5.1f degree') %(Xc, dphi)
...
Label(rf,text = '').pack(side=TOP, anchor=N) # spacer
...
Freq.insert(0,'150')
Label(f,text = _('Hz')).pack(side=LEFT)

f = Frame(rf)
f.pack(side=TOP, anchor = W)
Label(f,text = _('R = ')).pack(side=LEFT)
Res = Entry(f, width = 4)
...
Cap.insert(0,'1')
Label(f,text = _('uF')).pack(side=LEFT)

Calc = Button(rf,text=_('Calculate Xc and Phase shift'),command = calc)
Calc.pack(side=TOP)
Calc2 = Label(rf,text = '')
Calc2.pack(side=TOP)
#---------------
...
#set_frequency(0)
t = _('Steady state response of RC circuits')
eyeplot.pop_help('RCsteadystate', t)
root.title(t)
