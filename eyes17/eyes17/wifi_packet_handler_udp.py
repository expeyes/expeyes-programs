...
import socket, queue, threading, select
...
class WifiComms():
	...
	def __init__(self, ip, port):
		if ip == 'detect':
			...
			s.close()
			print(local_ip)
			self.server_address = (local_ip, 8080)
		else:
		...
		self.data_buffer = b''
	def check_data_waiting(self,sock):
		# Use select to check if data is waiting to be read
		readable, _, _ = select.select([sock], [], [], 0)
		return bool(readable)

	def read(self, bytes, who=None):
		st = time.time()
		while (time.time() - st) < self.timeout:
			if self.check_data_waiting(self.client_socket):
				data, server_address = self.client_socket.recvfrom(1024)
				if len(data):
					self.host_addr = server_address
					self.data_buffer += data

			if len(self.data_buffer) >= bytes:
				dat = self.data_buffer[:bytes]
				self.data_buffer = self.data_buffer[bytes:]
				#print('buffer nw ', self.data_buffer, 'need ', bytes, ' have ', len(self.data_buffer), 'from ', who)
				return dat
		print('timeout...')
		return b''
	...
	def readline(self):
		...
		while (time.time() - st) < self.timeout:
			if self.check_data_waiting(self.client_socket):
				...
				if b'\n' in self.data_buffer:
					dat = self.data_buffer.split(b'\n')
					self.data_buffer = dat[1]
					return dat[0]+b'\n'
