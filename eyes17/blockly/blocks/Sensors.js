...
// Super Duper Flexible Multiple Points dynamic I2C read
Blockly.Blocks['read_i2c_sensor_flexible_dynamic_array'] = {
  ...
  validateName: function(newValue) {
    this.currentValue = newValue;
    var configDropdown = this.getField('CONFIG');
    var paramtext = this.getField('PARAM');
    //
    try{
        if(typeof(HWBridge) !== 'undefined' && this.currentValue.startsWith("[")){
            var otherVal = this.currentValue;
            if(otherVal.search(']') != -1)otherVal = otherVal.split(']')[1];
            var x = JSON.parse(HWBridge.getSensorParameters(otherVal));
            console.log('PARAMETERS:'+x);
            paramtext.setValue('['+x+']');
        }
    }catch(e){    }
...
function getSensorList(){
            if(typeof(HWBridge) !== 'undefined'){
                        console.log('HWBridge available. update allsensors');
                        HWBridge.getAllSensors((vals)=>{
                            console.log('got following sensors',vals);
                            allsensors = JSON.parse(vals);
                            console.log('allsensors is now',allsensors);
                        });
            }else{
                console.log('HWBridge unavailable. could not update allsensors');
            }
...
function initSensors(interpreter, scope) {
		  ...
		  interpreter.setProperty(scope, 'get_generic_sensor', interpreter.createAsyncFunction(
				function(sensor,addr,callback) {
                    HWBridge.get_generic_sensor(sensor,addr,(vals)=>{callback(myInterpreter.nativeToPseudo(JSON.parse(vals)));});
				})
			);
		  ...
		  interpreter.setProperty(scope, 'parseAndReadSensor', interpreter.createAsyncFunction(
				function(descriptor,param,callback) {
                  ...
                  HWBridge.get_generic_sensor(sensor,parseInt(addr),(vals)=>{callback(myInterpreter.nativeToPseudo(JSON.parse(vals)[param]));});
				})
			);
		  ...
		  interpreter.setProperty(scope, 'parseAndReadSensorVals', interpreter.createAsyncFunction(
				function(descriptor,param,callback) {
                  addr = descriptor.split(']')[0].substring(1);
                  sensor = descriptor.split(']')[1].split('/')[0];
                  HWBridge.get_generic_sensor(sensor,parseInt(addr),(vals)=>{callback(myInterpreter.nativeToPseudo(JSON.parse(vals)));});
				})
			);
		  ...
		  interpreter.setProperty(scope, 'get_generic_sensor_param', interpreter.createAsyncFunction(
				function(sensor,addr, param,callback) {
				  HWBridge.get_generic_sensor_param(sensor,addr, param, (vals)=>{callback(myInterpreter.nativeToPseudo(vals));});
				})
			);
		  // Add an API for the get_sensor call
