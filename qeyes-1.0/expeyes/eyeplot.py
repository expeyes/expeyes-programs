...
PLOTBGCOL = 'white'
LINEWIDTH = 2 
LINECOL   = ['black', 'red', 'blue', 'green', 'cyan', 'magenta', 'yellow', 'orange','gray', 'gray2']
LABELCOL  = 'blue'
TEXTCOL   = 'black'
...
class graph:
	...
	def __init__(self, parent, width=400., height=300.,color = 'white', labels = True, bip=True):
		...
		if labels == False:
			f = Frame(self.parent, bg = 'black', borderwidth = self.border, relief = FLAT)#SUNKEN)
...
#--------------------------------------------------------------------------------
	...
	def setWorld(self, x1, y1, x2, y2, xlabel, ylabel):
		...
		try:
			...
			pass
		s = '%3.2f %s/div'%( (self.xmax-self.xmin)/NGRID1, xlabel)
...
def plot(x,y):			# plot the x,y coordinate list to a new , non-blocking, window.
	w = Tk()
	w.title('ExpEYES plot')
	g = graph(w)
	g.auto_scale(x,y)
	g.line(x,y)
	return g
...
img = None
def pop_image(sch, title = 'Schematic'):
	global img   	
	try:
		import Image, ImageTk, tkinter.font
		top = Toplevel()
