...
class Expt(QWidget):
	...
	def fit_curve(self):
		...
		if fa != None:
			try:
				self.p.set_state(10,1)		# Do some DC work to find the resistance of the Inductor
				time.sleep(.5)
				v = self.p.get_voltage(2)
				Vind = self.p.get_voltage(1)     # voltage across the Inductor
			except:
				self.comerr()
				return	
