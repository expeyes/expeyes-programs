...
def calc_g():
	...
	y,p=em.fit_qdr(x,y)
	g = p[0] * 2
	msgwin.config(text = 'Value of "g" by fitting the data points = %5.2f'%g)
...
def get_tof(index):
	try:
		...
		Results[index].config(text = 'Invalid H')
		return
	print index, h
	...
	if t > 0:
		...
		g = 2*h/t**2
		print g
		Results[index].config(text = 't=%6.4f g=%5.1f'%(t,g))
	else:
		Results[index].config(text = 'Timeout Err')
...
for k in range(NP):
	...
	b = Button(cf, text = 'Attach Ball at H=', command = attach)
