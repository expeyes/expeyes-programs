	...
	def start(self):
		...
		try:
			...
			if MINTEMP < tmp < MAXTEMP:
				...
				self.msg('temperature setpoint out of range')
				return
			self.MINY = 0
			...
			if .7 < upv <= 5.0:
				...
				self.msg('UPV setpoint out of range')
				return
			...
			self.msg('Starting the Controller')
			self.running = True
			p.write_outputs(1)
...
b3 = Label(cf, text = 'Ohm. ')
b3.pack(side = LEFT, anchor = SW)

b3 = Label(cf, text = 'UPV=')
b3.pack(side = LEFT, anchor = SW)
UPV = StringVar()
UPV.set('2.0')
