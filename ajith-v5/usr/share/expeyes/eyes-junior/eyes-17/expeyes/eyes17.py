...
class Interface():
	...
	* loads calibration values.
	...
	def __init__(self,timeout=1.0,**kwargs):
		...
		self.HX711_ready = False
		self.NRF_ready   = False
		self.OLED_ready = False
		self.HCSR01_ready = False
		...
		try:
			...
			if status&1 : self.NRF_ready = True
			if status&2 : self.OLED_ready = True
			if status&4 : self.HX711_ready = True
			if status&8 : self.HCSR01_ready = True
	...
	def __runInitSequence__(self,**kwargs):
		...
		#NRF wireless link
		if self.NRF_ready:
			from radio import NRF24L01,RadioLink
			self.NRF = NRF24L01(self.H)
			#print ('radio',self.NRF.get_status())
		#24-bit load cell sensor
		if self.HX711_ready:
			from HX711 import HX711
			self.HX711 = HX711(self.H)
	...
	def capture2(self,ns,tg,TraceOneRemap='A1',**kwargs):
		...
		**Arguments** 
		...
		:return: Arrays X(timestamps),Y1(Voltage at MIC),Y2(Voltage at CH2)
	...
	def __capture_fullspeed__(self,chan,samples,tg,*args,**kwargs):
		...
		**Arguments** 
		...
		.. code-block:: python
			...
			I.set_sqr1(40e3 , 50, True )   # Prepare a 40KHz, 50% square wave. Do not output it yet
			x,y = I.__capture_fullspeed__('A1',2000,1,'FIRE_PULSES',interval = 250) #Output the prepared 40KHz(25uS) wave for 250uS(10 cycles) before acquisition
			plot(x,y)               
			show()
...
#-------------------- Passive Time Interval Measurements ----------------------------------
	...
	def __capture_capacitance__(self,samples,tg):
		...
		try:
			...
			self.raiseException(ex, "Communication Error , Function : "+inspect.currentframe().f_code.co_name)
		try:
			fitres =  self.AC.fit_exp(x*1e-3,y)
			if fitres:
				cVal,newy = fitres
				#from pylab import *
				#plot(x,newy)
	...
	def capacitance_via_RC_discharge(self):
		...
		#print ('trying RC discharge curve fit. estimated cap: ',cap)
		T = 2*cap*20e3*1e6 #uS
		samples = 500
		try:
			if T>5000 and T<10e6:
				...
				return self.CAP_RC_SCALING*RC/10e3
			else:
