...
def setvoltage(n):
	...
	if error < ERRMAX:
		...
		ss ='Point %5.3fV. Deviation A0 %5.2f%%'%(v,error)
	else:
		msg('ERROR: Check BPV to A0 connection', 'red')
		return
	...
	if error < ERRMAX:
		...
		msg('ERROR: Check BPV to A1 connection')
		return
	Measured[n] = True
	msg(ss,'blue')
...
def calib():
	for k in range(NP):
		if Measured[k] == False:
			msg('ERROR : Point %5.3f Volts NOT done'%cpoints[k], 'red')
			return
		mv = float(mesVar[k].get())
		sv = cpoints[k]
		print sv, abs(sv-mv)
		if abs(sv-mv) > 0.1:
			msg('Readback for %5.3f V point = %5.3f V NOT GOOD'%(sv,mv), 'red')
			return
		advolts[k] = mv
	...
	msg('DONE: (%7.6f,%5.3f)(%7.6f,%5.3f)(%5.1f,%5.1f)'%(m0,c0,m1,c1,m8,c8) ,'blue')
